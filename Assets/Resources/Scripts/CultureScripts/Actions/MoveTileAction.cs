...
public class MoveTileAction : CultureMoveAction
{
    ...
    Turn AttemptMove()
    {
        if (prospectiveTile == null || prospectiveTile.GetComponent<TileInfo>().tileType == TileDrawer.BiomeType.Water || !(culture.affinity == prospectiveTile.GetComponent<TileInfo>().tileType || Random.value < moveChance)) // this is doing way too much work
        {
